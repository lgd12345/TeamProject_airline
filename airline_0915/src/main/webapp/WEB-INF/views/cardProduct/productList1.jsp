...
<!-- 	<div id="headerBottom">
 ...
 -->
	...
	<div id="container">
		<h3><a href="./productList">카드조회</a></h3>
	
		<!---- 검색 ---->
		<form id="frm" action="./${product}List1">
		...
		<div class="tab_wrap">
			<ul class="tabs">
				...
				<li class="tab-link" data-tab="tab-1"><a href="./productList?curPage=1&search=${pager.search}&kind=${pager.kind}">
						<span> 전체 </span>
				</a></li>
				<li class="tab-link current" data-tab="tab-2" id="EveryAir소식">
					<a href="./productList1?menu=EveryAir소식&curPage=1&search=${pager.search}&kind=${pager.kind}"> <span
						class="val"> EveryAir소식 </span></a>
				</li>
				
				<li class="tab-link" data-tab="tab-3" id="EveryAir클럽">
					<a href="./productList1?menu=EveryAir클럽&curPage=1&search=${pager.search}&kind=${pager.kind}"> 
					<span class="val"> EveryAir클럽 </span></a></li>
					
				<li class="tab-link" data-tab="tab-4" id="유류할증료">
					<a href="./productList1?menu=유류할증료&curPage=1&search=${pager.search}&kind=${pager.kind}"> <span> 유류할증료
					</span></a></li>
				<li class="tab-link" data-tab="tab-5" id="제휴사소식">
					<a href="./productList1?menu=제휴사소식&curPage=1&search=${pager.search}&kind=${pager.kind}"> <span> 제휴사소식
					</span></a></li>
				<li class="tab-link" data-tab="tab-6" id="기타">
					<a href="./productList1?menu=기타&curPage=1&search=${pager.search}&kind=${pager.kind}"> <span> 기타 </span></a></li>
			</ul>
			...
			<div id="tab-2" class="tab-content test current">
					...
					<ul>
						...
						<c:when test="${empty list and not empty pager.search}">
							...
							<div class="btn_wrap_ceType1">
								<a href="./productListList" id="btnList" type="button">전체리스트</a>
							</div>
						</div>				
						</c:when>
						...
						<c:forEach items="${list}" var="vo">
							<c:if test="${not empty vo.fix}">
								...
								<div class="left">
									<div class="title">
										<a href="./productListSelect?num=${vo.num}&dispCt=all&curPage=${pager.curPage}&search=${pager.search}&kind=${pager.kind}&menu=${param.menu}">
						...
						<c:if test="${empty vo.fix}">
							<li>
								<div class="left">
									<div class="title">
										<a href="./productListSelect?num=${vo.num}&menu=${param.menu}&curPage=${pager.curPage}&search=${pager.search}&kind=${pager.kind}">
											<em>${vo.title}</em>
										</a>
									</div>
									<p class="txt">${vo.textContents}</p>
				...
				<div class="paging">
					<a href="./productListList1?menu=${param.menu}&curPage=1"
						class="btn_first btn_common"></a>
					<c:choose>	
					<c:when test="${pager.curBlock>1}">
						<a
							href="./productListList1?menu=${param.menu}&curPage=${pager.startNum-1}"
							class="btn_prev btn_common"></a>
					</c:when>
					</c:choose>
					<c:forEach begin="${pager.startNum}" end="${pager.lastNum}" var="i">
						<a href="./productListList1?menu=${param.menu}&curPage=${i}"
							class="pagingNo" id="page${i}">${i}</a>
					</c:forEach>
					<c:choose>
					<c:when test="${pager.curBlock<pager.totalBlock}">
						<a href="./productListList1?menu=${param.menu}&curPage=${pager.lastNum+1}"
							class="btn_next btn_commont"></a>
					</c:when>
					</c:choose>
					<fmt:parseNumber var="pages" integerOnly="true" value="${tc/10}" />
					<a href="./productListList1?menu=${param.menu}&curPage=${pages+1}"
