			...
			<div class="list_view_ctrl">
				<div>
					<span class="prev">다음글</span>
					 <a href="./productSelect?num=${next.num}&menu=${param.menu}&curPage=${param.curPage}&search=${param.search}&kind=${param.kind}">${next.product}</a>
					<c:if test="${next eq null}"><a>다음글이 없습니다.</a></c:if>
				</div>
				<div>
					<span class="next">이전글</span>
					<a href="./productSelect?num=${prev.num}&menu=${param.menu}&curPage=${param.curPage}&search=${param.search}&kind=${param.kind}">${prev.product}</a>
					<c:if test="${prev eq null}"><a>이전글이 없습니다.</a></c:if>
				</div>
			</div>
			<c:if test="${member.id eq 'admin'}">
			<div class="btn_area">
					<a href="./productDelete?num=${vo.num}" id="delete_btn">삭제하기</a>
					<a href="./productUpdate?num=${vo.num}" id="update_btn">수정하기</a>
			</div>
			</c:if>
			<div class="btn_wrap">
					<c:if test="${member.id ne 'admin'}">
					<div class="btn_wrap">
					<a href="/cardProduct/productList" id="btnList" type="button">목록보기</a>
					</div>
					</c:if>
					
					<c:if test="${member.id eq 'admin'}">
					<div class="btn_wrap">
					<a href="/admin/admin5_1" id="btnList" type="button">목록보기</a>
					</div>
					</c:if>
			</div>
	</div>
