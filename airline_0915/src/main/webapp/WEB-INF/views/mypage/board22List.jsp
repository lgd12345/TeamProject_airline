...
<body>
		...
		<div id="hbh">
			...
			<div id="hbselect">
				<select onchange="location.href=this.value">
					...
					<option selected="selected" value="./notice22List">나의이용후기</option>
				</select>
			</div>
		</div>
	</div>
	...
	<div id="container">
		<h3>
			<a href="./notice22List">나의 이용 후기</a>
		</h3>
		...
		<div id="tab-1" class="tab-content current">
						...
						<c:when test="${empty list and not empty pager.search}">
							<div>
								...
								<div class="btn_wrap_ceType1">
									<a href="./notice22List" id="btnList" type="button">전체리스트</a>
								</div>
							</div>
						</c:when>
								...
								<c:if test="${not empty vo.fix}">
									<li class="bg_point">
										<div class="left">
											<div class="title">
												<a
													href="./notice22Select?num=${vo.num}&dispCt=all&curPage=${pager.curPage}&search=${pager.search}&kind=${pager.kind}">
			...
			<c:choose>
				<c:when test="${not empty list}">
					<div class="paging">
						<a href="./notice22List?curPage=1" class="btn_first btn_common">
						</a>

						<c:if test="${pager.curBlock>1}">
							<a href="./notice22List?curPage=${pager.startNum-1}"
								class="btn_prev btn_common"></a>
						</c:if>

						<c:forEach begin="${pager.startNum}" end="${pager.lastNum}"
							var="i">
							<a href="./notice22List?curPage=${i}" class="pagingNo"
								id="page${i}">${i}</a>
						</c:forEach>

						<c:if test="${pager.curBlock<pager.totalBlock}">
							<a href="./notice22List?curPage=${pager.lastNum+1}"
								class="btn_next btn_common"></a>
						</c:if>
						<fmt:parseNumber var="pages" integerOnly="true" value="${tc/10}" />
						<a href="./notice22List?curPage=${pages+1}"
