...
<option value="./limo">리무진 예약내역</option>
	...
	<div class="container">
		...
		<div class="list_view">
			...
			<div class="list_view_cont">${vo.contents}</div>
			<div class="list_view_ctrl">
				<div>
					<span class="prev">다음글</span> <a
						href="./notice2Select1?num=${next.num}&menu=${param.menu}&curPage=${param.curPage}&search=${param.search}&kind=${param.kind}">${next.title}</a>
					<c:if test="${next eq null}">
						<a>다음글이 없습니다.</a>
					</c:if>
				</div>
				<div>
					<span class="next">이전글</span> <a
						href="./notice2Select1?num=${prev.num}&menu=${param.menu}&curPage=${param.curPage}&search=${param.search}&kind=${param.kind}">${prev.title}</a>
			...
			<c:if test="${member.id eq vo.writer}">
				<div class="btn_area">
					<a href="./notice2Delete?num=${vo.num}" id="delete_btn">삭제하기</a> <a
						href="./notice2Update1?num=${vo.num}" id="update_btn">수정하기</a>
				</div>
			</c:if>
			<div class="btn_wrap">
				<c:if test="${member.id ne 'admin'}">
					<a href="./notice2List" id="btnList" type="button">목록보기</a>
				</c:if>
