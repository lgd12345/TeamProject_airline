...
<!---- container ---->
	...
	<div class="writeForm">
		...
		<form action="${board2}Update1" method="post" enctype="multipart/form-data" id="frm">
				...
				<tr>
					...
					<td>
						<select id="cate" name="cate" class="cate common textWrite">
							<option value="공지"<c:if test="${vo.cate eq '공지'}">selected</c:if>>공지</option>
							<option value="공항"disabled<c:if test="${vo.cate eq '공항'}">selected</c:if>>공항</option>
							<option value="비행기"disabled<c:if test="${vo.cate eq '비행기'}">selected</c:if>>비행기</option>	
							<option value="여행"disabled<c:if test="${vo.cate eq '여행'}">selected</c:if>>여행</option>
							<option value="기타"disabled<c:if test="${vo.cate eq '기타'}">selected</c:if>>기타</option>		
						</select>
					</td>
				</tr>
				...
				<tr>
					...
					<td>	 	
				 		<c:forEach var="i" begin="1" end="3">
				 			<div class="jfile_wrap">
								...
								<c:set var="j" value="${i}"></c:set>
									<input type="text" style="width: 646px" placeholder="파일첨부" value="${vo.notice2Files[j-1].oname}"
										 class="common addFileName" id="fileName${i}" readonly="readonly" >
									<button type="button"class="btn_detlete btn_delete${i}" id="${vo.notice2Files[j-1].fnum}">
					 	...
					 	<ul class= "list_type">
							...
							<li>ㆍ JPG, JPEG, PNG, 파일 형태로 첨부하시기 바랍니다.</li>
		...
		/* 	function uploadFile(file, editor) {
			...
			$('#attfile1').change(function(){
			 	 ...
			 	 if(window.FileReader){
					...
					$.post("./notice2FileDelete", {fnum:fnum}, function(data){
						data = data.trim();
