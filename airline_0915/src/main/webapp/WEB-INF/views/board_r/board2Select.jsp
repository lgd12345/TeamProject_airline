			...
			<div class="list_view_ctrl">
				<div>
					<span class="prev">다음글</span>
					 <a href="./notice2Select?num=${next.num}&menu=${param.menu}&curPage=${param.curPage}&search=${param.search}&kind=${param.kind}">${next.title}</a>
					<c:if test="${next eq null}"><a>다음글이 없습니다.</a></c:if>
				</div>
				<div>
					<span class="next">이전글</span>
					<a href="./notice2Select?num=${prev.num}&menu=${param.menu}&curPage=${param.curPage}&search=${param.search}&kind=${param.kind}">${prev.title}</a>
					<c:if test="${prev eq null}"><a>이전글이 없습니다.</a></c:if>
				</div>
			</div>
			<c:if test="${member.id eq vo.writer}">
			<div class="btn_area">
					<a href="./notice2Delete?num=${vo.num}" id="delete_btn">삭제하기</a>
					<a href="./notice2Update?num=${vo.num}" id="update_btn">수정하기</a>
			</div>
			</c:if>
			<div class="btn_wrap">
