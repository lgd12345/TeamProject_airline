let crawlingData = [];
const productInformationWrapper = document.querySelector(
    '.product-infomation-wrapper'
);
...
function setList(data) {
    productInformationWrapper.innerHTML = JSON.parse(data).reduce(
        (html = '', item, index = 0) => {
...
function reserveEvent() {
    document.querySelectorAll('.reserveButton').forEach((list, index) => {
        console.log(list);
        console.log(index);
        document.querySelector('.product-informaion');
...
function moveReserve(list, index) {
    list.addEventListener('click', function() {
        ...
        document.querySelector('.product-informaion').submit();
    });
}
...
function getList(data, index) {
    ...
    <div class="movie-rank">${data.productText}</div>
    <div class="poster-wrapper"><img src="${data.imgSrc}"></div>
    <div>
        <div class="movie-title">${data.productExpText}</div>
