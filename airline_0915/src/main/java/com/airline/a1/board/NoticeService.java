...
public class NoticeService {
	...
	public int noticeWrite(NoticeVO noticeVO, MultipartFile[] file) throws Exception {
		 ...
		 * if(file.length>0) {
		 * 
		 * for(MultipartFile multipartFile: file) { if(multipartFile.getSize()>0) {
		 * check = true; break; } }//for끝
		 * 
		 * if(check) { List<NoticeFilesVO> noticeFilesVOs = new ArrayList<>(); File
		 * file2 = filePathGenerator.getUseClassPathResource("board");
		 * 
		 * for(MultipartFile multipartFile:file) { if(multipartFile.getSize()>0) {
		 * String fileName = fileSaver.save2(file2, multipartFile); NoticeFilesVO
		 * noticeFilesVO = new NoticeFilesVO(); noticeFilesVO.setNum(noticeVO.getNum());
		 * noticeFilesVO.setFname(fileName);
		 * noticeFilesVO.setOname(multipartFile.getOriginalFilename());
		 * noticeFilesVOs.add(noticeFilesVO); } } result =
	...
	public int noticeUpdate(NoticeVO noticeVO, MultipartFile[] file, HttpSession session) throws Exception {
		 ...
		 * noticeFilesVO.setNum(noticeVO.getNum()); boolean check = false; int result =
		 * noticeMapper.noticeUpdate(noticeVO);
		 * 
		 * if(file.length>0) { for(MultipartFile multipartFile: file) {
		 * if(multipartFile.getSize()>0) { check = true; break; } }//for끝
		 * 
		 * if(check) { List<NoticeFilesVO> noticeFilesVOs = new ArrayList<>(); File
		 * file2 = filePathGenerator.getUseClassPathResource("board");
		 * 
		 * for(MultipartFile multipartFile:file) { if(multipartFile.getSize()>0) {
		 * String fileName = fileSaver.save(realPath, multipartFile); NoticeFilesVO
		 * noticeFilesVO = new NoticeFilesVO(); noticeFilesVO.setNum(noticeVO.getNum());
		 * noticeFilesVO.setFname(fileName);
		 * noticeFilesVO.setOname(multipartFile.getOriginalFilename());
		 * noticeFilesVOs.add(noticeFilesVO); } } result =
